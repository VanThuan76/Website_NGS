...
dockerBuildCommand = '. -f Dockerfile.uat'
...
pipeline {
    ...
    stage('Deploy and run') {
          steps {
            //sh 'chmod a+x ./2.deploy.sh'
            sh "helm upgrade --install ngs-website cicd/website -f ./cicd/website/values-uat.yaml --namespace ${namespace} --set image.tag=${TAG}"
            // sh './2.deploy.sh'
