...
export const roleSlice = createSlice({
  ...
  reducers: {
    ...
    setAllActionOfCategoryByType: (
      state,
      action: PayloadAction<{ categoryName: string; actionName: ActionPermission; value: boolean }>,
    ) => {
      ...
      if (cate) {
        cate.actionEditDto.forEach(per =>
          per.permissionActionDtoList
            ...
            .forEach(action => (action.active = value)),
        );
      }
    },
...
export const { setActionOfPermisison, setAllActionOfCategoryByType, setCatePermissions, setDefaultCheckPermission } =
  roleSlice.actions;
