...
export const useGetListNews = (defaultFilter?: Filter[]) => {
  return usePagination<IBaseResponse<IBaseResponseWithCount<IAdminNews[]>>>({
    queryKey: [QUERY_KEY],
    apiFn: params =>
      axiosInstance.post<IBaseResponse<IBaseResponseWithCount<IAdminNews[]>>>('/articles/search', { ...params }),
    defaultParams: {
      page: 0,
      size: 10,
...
export const useUpdateNews = (onSuccessHandle?: () => void) => {
  ...
  const { toast } = useToast();
  return useMutation({
    mutationFn: ({ id, news }: { id: React.Key; news: Partial<IAdminNews> }) =>
      axiosInstance.put(`/news/update/${id}`, news),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [QUERY_KEY] });
      if (onSuccessHandle) onSuccessHandle();
      toast({
