...
export const useGetListCategories = (defaultFilter?: Filter[]) => {
    return usePagination<IBaseResponse<IBaseResponseWithCount<ICategories[]>>>({
        queryKey: [QUERY_KEY],
        apiFn: (params) => axiosInstance.post<IBaseResponse<IBaseResponseWithCount<ICategories[]>>>('/categories/search', { ...params }),
        defaultParams: {
            page: 0,
            size: 10,
...
export const useUpdateCategories = (onSuccessHandle?: () => void) => {
    ...
    const { toast } = useToast()
    return useMutation({
        mutationFn: ({ id, categories }: { id: React.Key, categories: Partial<ICategories> }) => axiosInstance.put(`/categories/update/${id}`, categories),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: [QUERY_KEY] })
            if (onSuccessHandle) onSuccessHandle()
            toast({
...
export const useGetDetailCategoriesBySlug = ({ slug, options }: { slug: string, options: Partial<UseQueryOptions> }) => {
    return useQuery({
        queryKey: [QUERY_KEY],
