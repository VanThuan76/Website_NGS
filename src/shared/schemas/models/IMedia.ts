...
export const useGetListFolders = (defaultFilter?: Filter[]) => {
  return usePagination<IBaseResponse<IBaseResponseWithCount<Folders[]>>>({
    queryKey: [QUERY_KEY],
    apiFn: params =>
      axiosInstance.post<IBaseResponse<IBaseResponseWithCount<Folders[]>>>('/folders/search', { ...params }),
    defaultParams: {
      page: 0,
      size: 10,
...
export const useUpdateFolders = (onSuccessHandle?: () => void) => {
  ...
  const { toast } = useToast();
  return useMutation({
    mutationFn: ({ id, folders }: { id: React.Key; folders: Partial<Media> }) =>
      axiosInstance.put(`/folders/update/${id}`, folders),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [QUERY_KEY] });
      if (onSuccessHandle) onSuccessHandle();
      toast({
...
export const useGetListGalleries = (defaultFilter?: Filter[]) => {
  return usePagination<IBaseResponse<IBaseResponseWithCount<Galleries[]>>>({
    queryKey: [QUERY_KEY],
    apiFn: params =>
      axiosInstance.post<IBaseResponse<IBaseResponseWithCount<Galleries[]>>>('/galleries/search', { ...params }),
    defaultParams: {
      page: 0,
      size: 10,
      filters: defaultFilter,
...
export const useUpdateGalleries = (onSuccessHandle?: () => void) => {
  ...
  const { toast } = useToast();
  return useMutation({
    mutationFn: ({ id, galleries }: { id: React.Key; galleries: Partial<Media> }) =>
      axiosInstance.put(`/galleries/update/${id}`, galleries),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [QUERY_KEY] });
      if (onSuccessHandle) onSuccessHandle();
      toast({
