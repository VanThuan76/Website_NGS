...
export const useGetDetailComponentsByCode = ({
  code,
  options,
}: {
  code: React.Key;
...
export const useGetDetailComponentsBySectionCode = ({
  code,
  language,
  options,
}: {
  ...
  return useQuery({
    queryKey: [QUERY_KEY],
    queryFn: () =>
      axiosInstanceNoAuth.get<IBaseResponse<IBaseSectionComponent>>(
        `/components/get-by-section-code?language=${language}&code=${code}`,
      ),
    select(data) {
      return data.data;
    },
...
export const useGetDetailComponentsBySlug = ({
  slug,
  options,
}: {
  slug: string;
...
export const useGetDetailComponentsByCompany = ({
  id,
  options,
}: {
  id: React.Key;
...
export const useGetDetailComponentsBySection = ({
  id,
  options,
}: {
  id: React.Key;
...
export const useUpdateComponents = (onSuccessHandle?: () => void) => {
  ...
  const { trans } = useTrans();
  return useMutation({
    mutationFn: ({ components, id }: { components: IComponentsCreate; id: React.Key }) =>
      axiosInstanceNoAuth.put(`/components/update/${id}`, components),
    onSuccess: () => {
