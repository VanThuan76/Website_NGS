...
const formSchema = z.object({
  userName: z
    .string({ required_error: 'Vui lòng điền tên tài khoản' })
    .min(1, { message: 'Vui lòng điền tên tài khoản' }),
  fullName: z.string({ required_error: 'Vui lòng điền tên đầy đủ' }).min(1, { message: 'Vui lòng điền tên đầy đủ' }),
  phoneNumber: z
    .string({ required_error: 'Vui lòng điền số điện thoại' })
  ...
  organizationDto: z
    .string({ required_error: 'Vui lòng điền mã Id tổ chức' })
    .min(1, { message: 'Vui lòng điền mã Id tổ chức' }),
  ...
  useEffect(() => {
    if (defaultValue) {
      ...
      console.log(
        'data filtered' +
          JSON.stringify(
            (({ id, createdDate, updatedDate, createdBy, updatedBy, ...theRest }) => theRest)(defaultValue),
          ),
      );
